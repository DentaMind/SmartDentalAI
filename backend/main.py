...
def run_migrations_on_startup():
    ...
    if env == "development" and auto_migrate:
        logging.info("Development environment detected with AUTO_MIGRATE=true, running migrations...")
        try:
            ...
            if result.returncode == 0:
                ...
                logging.warning(f"Alembic migrations failed: {result.stderr}")
                # Don't raise an exception - continue app startup even if migrations fail
                # The DB version indicator will show the problem to admins
        except Exception as e:
            logging.error(f"Error running Alembic migrations: {e}")
    else:
        logging.info(f"Automatic migrations disabled: ENVIRONMENT={env}, AUTO_MIGRATE={auto_migrate}")
