...
class Settings(BaseSettings):
    ...
    # General settings
    ENVIRONMENT: str = Field("development", description="Current environment (development, staging, production)")
    ...
    # Security settings
    SECRET_KEY: SecretStr = Field(..., description="Secret key for tokens and encryption")
    ACCESS_TOKEN_EXPIRE_MINUTES: int = Field(60, description="JWT token expiration in minutes")
    PASSWORD_HASH_ALGORITHM: str = Field("bcrypt", description="Password hashing algorithm")
    
    # Database settings
    DATABASE_URL: PostgresDsn = Field(..., description="Database connection string")
    ...
    # Storage settings
    UPLOAD_DIR: str = Field("attached_assets/xrays", description="Directory for uploaded files")
