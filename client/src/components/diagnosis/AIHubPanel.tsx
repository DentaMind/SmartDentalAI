...
// Add a patient medical history interface
...
export default function AIHubPanel({ patientId, onScheduleAppointment }: AIHubPanelProps) {
  ...
  // Add state for patient medical history
  const [patientMedicalHistory, setPatientMedicalHistory] = useState<PatientMedicalHistory | null>(null);
  ...
  const loadAIHubData = async () => {
    setLoading(true);
    try {
      // Fetch all AI insights in parallel, including patient medical history
      const [riskRes, treatmentRes, financialRes, medicalHistoryRes] = await Promise.all([
      ...
      // Store the medical history
      setPatientMedicalHistory(medicalHistoryRes.data);
      
      // Enhance perio risk assessment with smoking status if available
      const riskData = riskRes.data;
      if (medicalHistoryRes.data && medicalHistoryRes.data.current_smoker) {
        // If patient is a smoker from the intake form, add this to perio risk factors
        if (!riskData.perioRisk.factors.some(factor => factor.toLowerCase().includes('smok'))) {
          riskData.perioRisk.factors.push('Patient is a current smoker (from medical history)');
          
          // If the risk is currently low, upgrade to moderate due to smoking
          if (riskData.perioRisk.level === 'low') {
            riskData.perioRisk.level = 'moderate';
            riskData.perioRisk.recommendations.push('Consider more frequent periodontal maintenance due to smoking status');
          }
          // If the risk is currently moderate, consider upgrading to high
          else if (riskData.perioRisk.level === 'moderate') {
            riskData.perioRisk.level = 'high';
            riskData.perioRisk.recommendations.push('Recommend smoking cessation counseling and more frequent periodontal assessment');
          }
        }
      }
              ...
              <>
                <div className="grid md:grid-cols-3 gap-4">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      {/* Clinical Note when smoking detected */}
                      {patientMedicalHistory?.current_smoker && (
                        <Alert className="mb-3 bg-amber-50 text-amber-800 border-amber-200">
                          <AlertTriangle className="h-4 w-4" />
                          <AlertTitle className="text-amber-800">Increased Perio Risk Due to Smoking</AlertTitle>
                          <AlertDescription className="text-amber-700 text-xs">
                            Recommend 3-4 month maintenance recall interval. Consider smoking cessation education materials.
                            <div className="mt-2">
                              <Button 
                                size="sm" 
                                variant="outline"
                                className="h-7 px-2 py-1 text-xs border-amber-300 bg-amber-50 hover:bg-amber-100 text-amber-800"
                      ...
                      <p className="text-sm">
                        {riskAssessment.perioRisk.details}
                      </p>
                      
                      {/* Risk Factors */}
                      <div className="mt-2">
                        <h4 className="text-xs font-medium mb-1">Risk factors:</h4>
                        <ul className="text-xs text-muted-foreground">
                          {riskAssessment.perioRisk.riskFactors.map((factor, i) => (
                            <li key={i} className="flex items-center gap-1 mb-0.5">
                              <div className="h-1.5 w-1.5 rounded-full bg-primary"></div>
                              {factor}
                            </li>
                          ))}
                          
                          {/* Add smoking as a risk factor if patient is a smoker */}
                          {patientMedicalHistory?.current_smoker && (
                            <li className="flex items-center gap-1 mb-0.5 font-medium text-amber-600">
