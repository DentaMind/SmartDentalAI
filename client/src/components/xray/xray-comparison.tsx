...
import { Brain, Layers, Download, Maximize, ArrowLeft, ArrowRight, Eye, EyeOff, ImageDown, Ruler, PanelTopOpen } from 'lucide-react';
...
export function XRayComparison({ beforeXray, afterXray, patientName, onClose }: XRayComparisonProps) {
  ...
  useEffect(() => {
    ...
    if (beforeImageRef.current) {
      beforeImageRef.current.style.transform = `scale(${zoomScale})`;
    }
    
    if (afterImageRef.current) {
      afterImageRef.current.style.transform = `scale(${zoomScale})`;
    }
  }, [zoom]);
  
  // Handle tissue overlay toggle
  useEffect(() => {
    // In a real implementation, this would apply a filter or overlay
    // to highlight soft tissue structures
    console.log('Tissue overlay:', showTissueOverlay);
  }, [showTissueOverlay]);
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button 
                          variant={showTissueOverlay ? "default" : "outline"} 
                          size="sm"
                          onClick={() => setShowTissueOverlay(!showTissueOverlay)}
                          className={showTissueOverlay ? "bg-primary text-primary-foreground" : ""}
                        >
