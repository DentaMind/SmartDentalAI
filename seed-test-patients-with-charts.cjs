  ...
  try {
    ...
    if (existingPatients.length > 0) {
      ...
      for (const chartData of perioCharts) {
        const [chart] = await db.insert(schema.periodontalCharts).values(chartData).returning();
      ...
      for (const chartData of restorativeChartsData) {
        const [chart] = await db.insert(schema.restorativeCharts).values(chartData).returning();
    ...
    const patientsForXrays = await db.select().from(schema.patients).limit(3);
    ...
    for (const patient of patientsForXrays) {
      ...
      for (const xrayData of patientXrays) {
        const [xray] = await db.insert(schema.xrays).values(xrayData).returning();
      ...
      const [note] = await db.insert(schema.medicalNotes).values(medicaNoteData).returning();
      ...
      const [plan] = await db.insert(schema.treatmentPlans).values(treatmentPlanData).returning();
