...
// Sample data for staff (dentists, hygienists, admin)
const staffMembers = [
  {
    username: 'dentist',
    password: 'password',
...
// Create SQL insertion commands for patients and staff
function generateSQLCommands() {
  ...
  const sqlFilePath = path.join(sqlDir, 'insert_test_data.sql');
  let sql = '';

  // Insert staff users first (dentists, hygienists, admin)
  staffMembers.forEach((staff, index) => {
    sql += `-- Create user for ${staff.firstName} ${staff.lastName} (${staff.role})\n`;
    sql += `INSERT INTO users (username, "passwordHash", email, "firstName", "lastName", role, "mfaSecret", "mfaEnabled", "specialization", "licenseNumber", "createdAt", "updatedAt")\n`;
    sql += `VALUES ('${staff.username}', '$2b$10$LqXB9wMxHUVLYG0nGQfRjuKLQX4iKZ0BHLiID22v70iyZXrsiCVvC', '${staff.email}', '${staff.firstName}', '${staff.lastName}', '${staff.role}', '', false, ${staff.specialization ? `'${staff.specialization}'` : 'NULL'}, ${staff.licenseNumber ? `'${staff.licenseNumber}'` : 'NULL'}, NOW(), NOW());\n\n`;
  });
  ...
  // Insert patient records linked to users
  testPatients.forEach((patient, index) => {
...
async function generateSampleData() {
  ...
  try {
    ...
    staffMembers.forEach(staff => {
      console.log(`- ${staff.firstName} ${staff.lastName} (${staff.role}) - ${staff.username}/password`);
    });
    ...
    console.log('\nAll staff accounts use email: dentamind27@gmail.com');
